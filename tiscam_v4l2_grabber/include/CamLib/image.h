...
public:
    ...
    uint GetIndex(const uint x,const uint y)const {
#ifdef SAFE_IMAGE_ACCESS
    ...
    inline int GetIndexRC(const float x,const float y)const {
    ...
    const DataType & operator ()(const uint x,const uint y)const{
        return data[GetIndex(x,y)];
    }
    ...
    Image<DataType> subsample (int step){

        Image<DataType> img({size.w/step,size.h/step});

        for(int x=0,xo=0;x<img.size.w;x++,xo+=step)
            for(int y=0,yo=0;y<img.size.h;y++,yo+=step)
                img(x,y)=(*this)(xo,yo);
        return img;
